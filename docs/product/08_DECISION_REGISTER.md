...
| D-014 | Provisional | DeepSeek Harness is the production host. | Host-lifecycle feasibility passed on Windows, Linux and macOS with DSH 0.1.6-alpha.1 (2026-09-25); agent-facing seams (session events, tools, UI) are still unverified. |
...
| D-031 | Accepted | Supported platforms for the feasibility plugin with DSH 0.1.6-alpha.1: Windows x64, Linux x64, macOS arm64 on Node 22.19+ and 24. | Green CI matrix run 36116878136 (2026-09-25); partial-coverage is inconclusive on elevated Windows accounts. Other versions/architectures are unclaimed. |
...
| D-034 | Accepted | `ctx.roundtableCodeHealth` v1 is an internal read/refresh service with no authority; events carry counts only, no evidence text. `requestScan` never throws synchronously (validation errors reject), and never reads through symlinked path segments. | Keeps findings evidence out of model-visible or cross-plugin channels until an audited seam exists; independent review found an out-of-workspace read via junctions, fixed 2026-09-25. |
