export class FindingLedger {
  ...
  observe(finding) {
    // A second observation of the same fingerprint in one run (e.g. overlapping scan targets)
    // must not reclassify it: the first observation already compared it with the prior run.
    if (this.#seen.has(finding.fingerprint)) return this.#records.get(finding.fingerprint)
